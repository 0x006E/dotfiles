...
delib.module {
  ...
  nixos.ifEnabled =
    ...
    in
    {
      ...
      # API token for opencode's MCP bridge (minted in Vikunja UI → API
      # tokens, stored encrypted in secrets/secrets.yaml). Exported into
      # interactive shells in programs.cli; never lands in the nix store.
      # NOTE: `rs` fails until the `vikunja_api_token` key exists in the
      # sops file — add it first (see issue-tracker.md).
      sops.secrets.vikunja_api_token = {
        owner = username;
        mode = "0400";
      };
    };
