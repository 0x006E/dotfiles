...
delib.module {
  ...
  nixos.always = { myconfig, ... }: { pkgs, config, lib, pkgs-stable, pkgs-unstable, ... }: {
    imports = [ inputs.determinate.nixosModules.default inputs.nix-index-database.nixosModules.nix-index ];
    home-manager = {
      useGlobalPkgs = true;
